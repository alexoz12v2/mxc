...
#include <cstdint>
	...
	#define MXC_DEBUG(message, ...) ::mxc::core::logOutput(::mxc::LogLevel::debug, __FILE__, __LINE__, message, ##__VA_ARGS__)
#else
	#define MXC_DEBUG(...)
#endif
	...
	#define MXC_ERROR(message, ...) ::mxc::core::logOutput(::mxc::LogLevel::error, __FILE__, __LINE__, message, ##__VA_ARGS__)
#else
	#define MXC_ERROR(...)
#endif

#if defined(MXC_LOGLEVEL_TRACE)
	#define MXC_TRACE(message, ...) ::mxc::core::logOutput(::mxc::LogLevel::trace, __FILE__, __LINE__, message, ##__VA_ARGS__)
#else
	#define MXC_TRACE(...)
#endif

#if defined(_DEBUG) || defined(MXC_DEBUG)
	#define MXC_ASSERT(condition, message, ...) ::mxc::core::assertCondition(__FILE__, __LINE__, (condition), (message), ##__VA_ARGS__)
#else
	#define MXC_ASSERT(...)
#endif
