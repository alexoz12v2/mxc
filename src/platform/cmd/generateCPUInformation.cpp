...
// NOTE: When adding new architectures, still define all the other namespaces and defines with all the relative values, because somewhere else
// (eg simd.h) the defines are used for the purpose of conditional compilation, and #if doesn't short circuit!
auto main() -> int32_t
{
...
namespace mxc::platform
{
	...
	char const* features_x86_64[features_x86_64_count] = {
        "bSSE",
        "bSSE2",
        "bSEE3",
        "bSupplementalSSE3",
    ...
    bool featureSupport_x86_64[features_x86_64_count] = {
        features1.edx.bSSE(),
        features1.edx.bSSE2(),
        features1.ecx.bSSE3(),
    ...
    printf("// support information:\n//\tcpuid(eax=1):      edx=0x%x, ecx=0x%x\n//\tcpuid(eax=7,ecx=0): ebx=0x%x, ecx=0x%x, edx=0x%x\n//\t"
           "cpuid(eax=7,ecx=1): eax=0x%x, ebx=0x%x, edx=0x%x\n",
           features1.edx.reg, features1.ecx.reg, features70.ebx.reg, 
           features70.ecx.reg, features70.edx.reg, features71.eax.reg, features71.ebx.reg, features71.edx.reg);
    printf("\n// preprocessor definitions:\n");

    for (uint32_t i = 0; i != features_x86_64_count; ++i)
    {
        printf("#define %s %s\n", features_x86_64_defines[i], falseTrue[static_cast<uint32_t>(featureSupport_x86_64[i])]);
    }
#endif
    ...
    for (uint32_t i = 0; i != features_x86_64_count; ++i)
    {
        printf("\t\t%s %s = %s;\n", qualifiers, features_x86_64[i], falseTrue[static_cast<uint32_t>(featureSupport_x86_64[i])]);
    }
