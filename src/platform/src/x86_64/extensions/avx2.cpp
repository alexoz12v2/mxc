...
{
    v256i16 const vRes = _mm256_blend_epi16(v0, v1, ctrl);
    return vRes;
}
template <int32_t ctrl> requires ((ctrl & 0x0000'00ff) == 0)
MXC_VECTOR_ABI auto choose_i32(v128i32 v0, v128i32 v1) -> v128i32        // _mm_blend_epi32
{
    v128i32 const vRes = _mm_blend_epi32(v0, v1, ctrl);
    return vRes;
}
template <int32_t ctrl> requires ((ctrl & 0x0000'00ff) == 0)
MXC_VECTOR_ABI auto choose_i32(v256i32 v0, v256i32 v1) -> v256i32        // _mm256_blend_epi32
{
    v256i32 const vRes = _mm256_blend_epi32(v0, v1, ctrl);
    return vRes;
}

// _mm256_blendv_epi8
MXC_VECTOR_ABI auto chooseVariable_i8(v256i8 v0, v256i8 v1, v256ix vMask) -> v256i8 
{
    v256i8 const vRes = _mm256_blendv_epi8(v0, v1, vMask);
    return vRes;
}
...
{
    v128ix const vRes = _mm256_extracti128_si256(v, lane);
    return vRes;
}
...
{
    v128i32 const vRes =_mm_mask_i32gather_epi32(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherd_i32(
...
{
    v256i32 const vRes = _mm256_mask_i32gather_epi32(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128i64 const vRes = _mm_mask_i32gather_epi64(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherd_i64(
...
{
    v256i64 const vRes = _mm256_mask_i32gather_epi64(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128d const vRes = _mm_mask_i32gather_pd(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherd_d(
...
{
    v256d const vRes = _mm256_mask_i32gather_pd(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128f const vRes = _mm_mask_i32gather_ps(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherd_f(
...
{
    v256f const vRes = _mm256_mask_i32gather_ps(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128i32 const vRes = _mm_mask_i64gather_epi32(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherq_i32(
...
{
    v128i32 const vRes = _mm256_mask_i64gather_epi32(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128i64 const vRes = _mm_mask_i64gather_epi64(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherq_i64(
...
{
    v256i64 const vRes = _mm256_mask_i64gather_epi64(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128d const vRes = _mm_mask_i64gather_pd(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherq_d(
...
{
    v256d const vRes = _mm256_mask_i64gather_pd(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
...
{
    v128f const vRes = _mm_mask_i64gather_ps(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
template <int32_t scale> requires (scale == 1 || scale == 2 || scale == 4 || scale == 8)
MXC_VECTOR_ABI auto gatherq_f(
...
{
    v256f const vRes = _mm256_mask_i64gather_ps(vSrc, pBase, vIndices, vMask, scale);
    return vRes;
}
...
{
    v256ix const vRes = _mm256_inserti128_si256(vSrc, vOpr, lane);
    return vRes;
}

MXC_VECTOR_ABI auto fmaddAdjacent_i16(v256i16 v0, v256i16 v1) -> v256i32  // _mm256_madd_epi16
{
    v256i32 const vRes = _mm256_madd_epi16(v0, v1);
    return vRes;
}
...
{
    v256i16 const vRes = _mm256_maddubs_epi16(v0, v1);
    return vRes;
}

// masked elements are zeroed out                                       // _mm_maskload_epi32
MXC_VECTOR_ABI auto maskload_i32(int32_t const *MXC_RESTRICT iarr, v128ix vMask) -> v128i32 
{
    v128i32 const vRes = _mm_maskload_epi32(iarr, vMask);
    return vRes;
}
                                                                        // _mm256_maskload_epi32
MXC_VECTOR_ABI auto maskload_i32(int32_t const *MXC_RESTRICT iarr, v256ix vMask) -> v256i32 
{
    v256i32 const vRes = _mm256_maskload_epi32(iarr, vMask);
    return vRes;
}
                                                                        // _mm_maskload_epi64
MXC_VECTOR_ABI auto maskload_i64(int64_t const *MXC_RESTRICT iarr, v128ix vMask) -> v128i64 
{
    v128i64 const vRes = _mm_maskload_epi64(
        reinterpret_cast<long long int const *MXC_RESTRICT>(iarr), // shut compiler up
        vMask);
    return vRes;
}
                                                                        // _mm256_maskload_epi64
MXC_VECTOR_ABI auto maskload_i64(int64_t const *MXC_RESTRICT iarr, v256ix vMask) -> v256i64 
{
    v256i64 const vRes = _mm256_maskload_epi64(
        reinterpret_cast<long long int const *MXC_RESTRICT>(iarr), // shut compiler up
        vMask);
    return vRes;
}
...
MXC_VECTOR_ABI auto maskstore_i32(v128i32 v, int32_t *MXC_RESTRICT iarr, v128ix vMask) -> void 
{
    _mm_maskstore_epi32(iarr, vMask, v);
}
                                                                        // _mm256_maskstore_epi32
MXC_VECTOR_ABI auto maskstore_i32(v256i32 v, int32_t *MXC_RESTRICT iarr, v256ix vMask) -> void 
{
    _mm256_maskstore_epi32(iarr, vMask, v);
}
                                                                        // _mm_maskstore_epi64
MXC_VECTOR_ABI auto maskstore_i64(v128i64 v, int64_t *MXC_RESTRICT iarr, v128ix vMask) -> void 
{
    _mm_maskstore_epi64(reinterpret_cast<long long int *MXC_RESTRICT>(iarr), vMask, v);
}
                                                                        // _mm256_maskstore_epi64
MXC_VECTOR_ABI auto maskstore_i64(v256i64 v, int64_t *MXC_RESTRICT iarr, v256ix vMask) -> void 
{
    _mm256_maskstore_epi64(reinterpret_cast<long long int *MXC_RESTRICT>(iarr), vMask, v);
}

MXC_VECTOR_ABI auto max_i8(v256i8 v0, v256i8 v1) -> v256i8      // _mm256_max_epi8
{
    v256i8 const vRes = _mm256_max_epi8(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto max_i16(v256i16 v0, v256i16 v1) -> v256i16  // _mm256_max_epi16
{
    v256i16 const vRes = _mm256_max_epi16(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto max_i32(v256i32 v0, v256i32 v1) -> v256i32  // _mm256_max_epi32
{
    v256i32 const vRes = _mm256_max_epi32(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto max_u8(v256u8 v0, v256u8 v1) -> v256u8      // _mm256_max_epu8
{
    v256u8 const vRes = _mm256_max_epu8(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto max_u16(v256u16 v0, v256u16 v1) -> v256u16  // _mm256_max_epu16
{
    v256u16 const vRes = _mm256_max_epu16(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto max_u32(v256u32 v0, v256u32 v1) -> v256u32  // _mm256_max_epu32
{
    v256u32 const vRes = _mm256_max_epu32(v0, v1);
    return vRes;
}

MXC_VECTOR_ABI auto min_i8(v256i8 v0, v256i8 v1) -> v256i8      // _mm256_min_epi8
{
    v256i8 const vRes = _mm256_min_epi8(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto min_i16(v256i16 v0, v256i16 v1) -> v256i16  // _mm256_min_epi16
{
    v256i16 const vRes = _mm256_min_epi16(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto min_i32(v256i32 v0, v256i32 v1) -> v256i32  // _mm256_min_epi32
{
    v256i32 const vRes = _mm256_min_epi32(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto min_u8(v256u8 v0, v256u8 v1) -> v256u8      // _mm256_min_epu8
{
    v256u8 const vRes = _mm256_min_epu8(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto min_u16(v256u16 v0, v256u16 v1) -> v256u16  // _mm256_min_epu16
{
    v256u16 const vRes = _mm256_min_epu16(v0, v1);
    return vRes;
}
MXC_VECTOR_ABI auto min_u32(v256u32 v0, v256u32 v1) -> v256u32  // _mm256_min_epu32
{
    v256u32 const vRes = _mm256_min_epu32(v0, v1);
    return vRes;
}

MXC_VECTOR_ABI auto grabMSBs(v256ix v) -> int32_t   // _mm256_movemask_epi8
{
    int32_t const mask = _mm256_movemask_epi8(v);
    return mask;
}
...
{
    v256u16 const vSad = _mm256_mpsadbw_epu8(vSrc, vKern, bits);
    return vSad;
}
...
{
    v256ix const vRes = _mm256_permute2x128_si256(v0, v1, lane);
    return vRes;
}
...
{
    v256i64 const vRes = _mm256_permute4x64_epi64(vSrc, lane);
    return vRes;
}
template <int32_t lane> requires (lane >= 0 && lane < 4)
MXC_VECTOR_ABI auto permute_d(v256d v) -> v256d                  // _mm256_permute4x64_pd
{
    v256d const vRes = _mm256_permute4x64_pd(v, lane);
    return vRes;
}

MXC_VECTOR_ABI auto permute_f(v256f v, v256ix vIdxs) -> v256f    // _mm256_permutevar8x32_ps
{
    v256f const vRes = _mm256_permutevar8x32_ps(v, vIdxs);
    return vRes;
}

MXC_VECTOR_ABI auto permute_i32(v256i32 v, v256ix vIdxs) -> v256i32  // _mm256_permutevar8x32_epi32
{
    v256i32 const vRes = _mm256_permutevar8x32_epi32(v, vIdxs);
    return vRes;
}
...
{
    v256i32 const vRes = _mm256_shuffle_epi32(v, ctrl);
    return vRes;
}

MXC_VECTOR_ABI auto shuffle_i8(v256i8 v, v256i8 vIdxs) -> v256i8  // _mm256_shuffle_epi8
{
    v256i8 const vRes = _mm256_shuffle_epi8(v, vIdxs);
    return vRes;
}
...
{
    v256i16 const vRes = _mm256_slli_epi16(v, shamt);
    return vRes;
}
template <int32_t shamt> requires ((shamt & 0x0000'00ff) == 0)
MXC_VECTOR_ABI auto lshift_i32(v256i32 v) -> v256i32    // _mm256_slli_epi32
{
    v256i32 const vRes = _mm256_slli_epi32(v, shamt);
    return vRes;
}
template <int32_t shamt> requires ((shamt & 0x0000'00ff) == 0)
MXC_VECTOR_ABI auto lshift_i64(v256i64 v) -> v256i64    // _mm256_slli_epi64
{
    v256i64 const vRes =_mm256_slli_epi64(v, shamt);
    return vRes;
}
...
{
    v256i16 const vRes = _mm256_srli_epi16(v, shamt);
    return vRes;
}
template <int32_t shamt> requires ((shamt & 0x0000'00ff) == 0)
MXC_VECTOR_ABI auto rshift_i32(v256i32 v) -> v256i32    // _mm256_srli_epi32
{
    v256i32 const vRes = _mm256_srli_epi32(v, shamt);
    return vRes;
}
template <int32_t shamt> requires ((shamt & 0x0000'00ff) == 0)
MXC_VECTOR_ABI auto rshift_i64(v256i64 v) -> v256i64    // _mm256_srli_epi64
{
    v256i64 const vRes = _mm256_srli_epi64(v, shamt);
    return vRes;
}
