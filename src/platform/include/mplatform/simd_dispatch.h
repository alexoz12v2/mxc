...
namespace mxc::simd
{
    ...
    enum class SIMDSupport : uint32_t
    {
    #if defined(MXC_ARCH_X86_64)
    ...
    {
        return static_cast<SIMDSupport>(std::underlying_type<SIMDSupport>::type(a) | std::underlying_type<SIMDSupport>::type(b));
    }
    ...
    {
        return (std::underlying_type<SIMDSupport>::type(a) != 0);
    }
     ...
     *  - in 'namespace detail', under the relevant architecture section, declare the vectorized overload of the function, which are returned by 
     *    the dispatcher
     *  - as a patch for msvc, which doesn't support indirect functions, whose overload is resolved by calling the dispatcher at load time,
     *    global, inline pointers named g_<func name>_func are defined by initializing them calling their associated dispatcher.
     *    Furthermore, the actual function declared in the third step is defined as empty
     * Usage:
     *      MXC_SIMD_DISPATCH(<func name>, args...)
     */
    typedef MXC_VECTOR_ABI void addfv_t(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    MXC_SIMD_DISPATCHER_ATTRIB(addfv_dispatch)
    MXC_VECTOR_ABI void addfv(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    ...
    typedef MXC_VECTOR_ABI void subfv_t(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    MXC_SIMD_DISPATCHER_ATTRIB(subfv_dispatch)
    MXC_VECTOR_ABI void subfv(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    ...
    typedef MXC_VECTOR_ABI void mulfv_t(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    MXC_SIMD_DISPATCHER_ATTRIB(mulfv_dispatch)
    MXC_VECTOR_ABI void mulfv(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    ...
    typedef MXC_VECTOR_ABI void divfv_t(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    MXC_SIMD_DISPATCHER_ATTRIB(divfv_dispatch)
    MXC_VECTOR_ABI void divfv(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len);
    ...
    // functions
        ...
        MXC_VECTOR_ABI void addfv_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_WARN("AVX 256 bit operations should be used for long sequences of relatively cheap operations (i.e. not sqrt/div), as\n\t1) "
                     "using AVX for one, cheap operation, will likely result in getting bandwidth limited\n\t2) Slow operations have lower latency on "
                     "SSE(2,3,4.1,4.2, non-VEX or VEX)");
            MXC_ASSERT((len%8==0),"[x86_64] to enable avx vectorization float32 vectors of length multiple of 8 is required");
        ...
        MXC_VECTOR_ABI void addfv_sse2(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_ASSERT(len % 4 == 0, "[x86_64] to enable sse2 vectorization float32 vectors of length multiple of 4 is required");
            for (uint32_t i = 0; i < len; i += 4)
            {
                v128f va = sse2::load_f(a + i * sizeof(float));
                v128f vb = sse2::load_f(b + i * sizeof(float));
                v128f vRes = sse2::add_f(va, vb);
                sse2::store_f(vRes, res);
            }
        }
        ...
        MXC_VECTOR_ABI void addf2_sse2(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res)
        {
            ...
            v128f va = sse2::load_f(fa);
            v128f vb = sse2::load_f(fb);
            v128f vRes = sse2::add_f(va, vb);
            sse2::store_f(vRes, fres);
            res[0] = fres[0];
            res[1] = fres[1];
        }
        ...
        MXC_VECTOR_ABI void addf2_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res)
        {
            ...
            v128f va = sse2vex::load_f(fa);
            v128f vb = sse2vex::load_f(fb);
            v128f vRes = sse2vex::add_f(va, vb);
            sse2vex::store_f(vRes, fres);
            res[0] = fres[0];
            res[1] = fres[1];
        }
        ...
        {
            v128f va = sse2::load_f(a);
            v128f vb = sse2::load_f(b);
            v128f vRes = sse2::add_f(va, vb);
            sse2::store_f(vRes, res);
        }
        ...
        {
            v128f va = sse2vex::load_f(a);
            v128f vb = sse2vex::load_f(b);
            v128f vRes = sse2vex::add_f(va, vb);
            sse2vex::store_f(vRes, res);
        }

        MXC_VECTOR_ABI void subfv_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_WARN("AVX 256 bit operations should be used for long sequences of relatively cheap operations (i.e. not sqrt/div), as\n\t1) "
                     "using AVX for one, cheap operation, will likely result in getting bandwidth limited\n\t2) Slow operations have lower latency on "
                     "SSE(2,3,4.1,4.2, non-VEX or VEX)");
        ...
        MXC_VECTOR_ABI void subfv_sse2(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_ASSERT(len % 4 == 0, "[x86_64] to enable sse2 vectorization float32 vectors of length multiple of 4 is required");
            for (uint32_t i = 0; i < len; i += 4)
            {
        ...
        MXC_VECTOR_ABI void subf2_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res)
        {
            ...
            v128f va = sse2vex::load_f(fa);
            v128f vb = sse2vex::load_f(fb);
            v128f vRes = sse2vex::sub_f(va, vb);
            sse2vex::store_f(vRes, fres);
            res[0] = fres[0];
            res[1] = fres[1];
        }
        ...
        MXC_VECTOR_ABI void mulfv_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_WARN("AVX 256 bit operations should be used for long sequences of relatively cheap operations (i.e. not sqrt/div), as\n\t1) "
                     "using AVX for one, cheap operation, will likely result in getting bandwidth limited\n\t2) Slow operations have lower latency on "
                     "SSE(2,3,4.1,4.2, non-VEX or VEX)");
        ...
        MXC_VECTOR_ABI void mulfv_sse2(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_ASSERT(len % 4 == 0, "[x86_64] to enable sse2 vectorization float32 vectors of length multiple of 4 is required");
            for (uint32_t i = 0; i < len; i += 4)
            {
        ...
        MXC_VECTOR_ABI void mulf2_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res)
        {
            ...
            v128f va = sse2vex::load_f(fa);
            v128f vb = sse2vex::load_f(fb);
            v128f vRes = sse2vex::mul_f(va, vb);
            sse2vex::store_f(vRes, fres);
            res[0] = fres[0];
            res[1] = fres[1];
        }
        ...
        MXC_VECTOR_ABI void divfv_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_WARN("AVX 256 bit operations should be used for long sequences of relatively cheap operations (i.e. not sqrt/div), as\n\t1) "
                     "using AVX for one, cheap operation, will likely result in getting bandwidth limited\n\t2) Slow operations have lower latency on "
                     "SSE(2,3,4.1,4.2, non-VEX or VEX)");
            MXC_ASSERT((len%8==0),"[x86_64] to enable avx vectorization float32 vectors of length divtiple of 8 is required");
            for (uint32_t i = 0; i < len; i += 8)
            {
                v256f va = avx::load_f(a + i * sizeof(float));
                v256f vb = avx::load_f(b + i * sizeof(float));
        ...
        MXC_VECTOR_ABI void divfv_sse2(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            MXC_ASSERT(len % 4 == 0, "[x86_64] to enable sse2 vectorization float32 vectors of length divtiple of 4 is required");
            for (uint32_t i = 0; i < len; i += 4)
            {
                v128f va = sse2::load_f(a + i * sizeof(float));
                v128f vb = sse2::load_f(b + i * sizeof(float));
                v128f vRes = sse2::div_f(va, vb);
                sse2::store_f(vRes, res);
            }
        }
        ...
        MXC_VECTOR_ABI void divf2_avx(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res)
        {
            ...
            v128f va = sse2vex::load_f(fa);
            v128f vb = sse2vex::load_f(fb);
            v128f vRes = sse2vex::div_f(va, vb);
            sse2vex::store_f(vRes, fres);
            res[0] = fres[0];
            res[1] = fres[1];
        }
        ...
        MXC_VECTOR_ABI void addfv_default(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
        ...
        MXC_VECTOR_ABI void subfv_default(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            for (uint32_t i = 0; i < len; ++i)
            {
                res[i] = a[i] - b[i];
        ...
        MXC_VECTOR_ABI void mulfv_default(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            for (uint32_t i = 0; i < len; ++i)
            {
                res[i] = a[i] * b[i];
        ...
        MXC_VECTOR_ABI void divfv_default(float const* MXC_RESTRICT a, float const* MXC_RESTRICT b, float* MXC_RESTRICT res, uint32_t len)
        {
            for (uint32_t i = 0; i < len; ++i)
            {
                res[i] = a[i] / b[i];
    ...
    // ifunc needs to be declared with the mangled name of the dispatcher function. To avoid headaches, using C linking to avoid name mangling 
    // all together
#if defined(MXC_ARCH_X86_64)
