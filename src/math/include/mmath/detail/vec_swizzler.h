...
namespace mxc::math
{
    ...
    template <uint32_t I, uint32_t N, uint32_t... R>
    struct pick : pick <I - 1, R...> { };

    template <uint32_t N, uint32_t... R>
    struct pick <0, N, R...> : std::integral_constant<uint32_t, N> { };

    template <uint32_t I, uint32_t N, uint32_t... R>
    inline constexpr auto pick_v = pick<I,N,R...>::value;
    ...
    template <typename T> struct Vec_withSize<T,1>{using type = T;};
    template <typename T> struct Vec_withSize<T,2>{using type = Vector<T,0,1>;};
    template <typename T> struct Vec_withSize<T,3>{using type = Vector<T,0,1,2>;};
    template <typename T> struct Vec_withSize<T,4>{using type = Vector<T,0,1,2,3>;};

    //Target Vector Type: This is the type of vector that can be assigned from the swizzled class. For example, when you swizzle components 
    ...
    //Size of Original Vector: This indicates the size of the original vector that you are swizzling from, such as 1 for vec1, 2 for vec2, 
    //  3 for vec3, and 4 for vec4.
    //Component Indices: These are the indices of the components you are selecting to form the swizzle. For example, xy represents the swizzle 
    //  of the x and y components.
    template <typename VecT, typename T, uint32_t vecDims, uint32_t... accessors>
    struct Swizzler
    {
        ...
        constexpr operator VecT() const
        {
            ...
            if constexpr (std::floating_point<decltype(v)> || std::integral<decltype(v)>)
            {
                ...
                unroll<0,vecDims>([&]<uint32_t i>(){
                    v[i] = data[pick_v<i, accessors...>];
                });
            }
            return v;
        }
