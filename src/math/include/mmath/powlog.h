...
MXC_FORCEINLINE constexpr auto exp(float const x) -> float
{
    ...
    float const taylor4 =
        1 + small + (12 * small * small + 4 * small * small * small + small * small * small * small) / 24;
    float const y = k >= 0 ? (taylor4 * (1 << static_cast<uint32_t>(k))) : (taylor4 / (1 << static_cast<uint32_t>(-k)));
...
{
    MMATH_ASSERT_FINITE_NORMALIZED_FLOAT(x);
    assert(x <= std::numeric_limits<float>::epsilon() && "function 'expm1' expects a small number");
...
{
    MMATH_ASSERT_FINITE_NORMALIZED_FLOAT(x);
    assert(x > std::numeric_limits<float>::epsilon() && "function 'ln' requires a positive, not too small, number");
...
MXC_FORCEINLINE auto constexpr twoxp(float x) -> float
{
    ...
    taylor_rtaylor[0] =
        1.f + x * (detail::ln2 + x * (detail::ln2pow2halved + x * (detail::ln2pow3div6 + x * detail::ln2pow4div24)));
    taylor_rtaylor[1] =
        1.f /
        (1.f + x * (detail::ln2 + x * (detail::ln2pow2halved + x * (detail::ln2pow3div6 + x * detail::ln2pow4div24))));
