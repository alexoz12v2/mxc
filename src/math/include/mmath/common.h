...
{
    // De Brujin sequence
    char constexpr bitPosition[64] = {0,  1,  2,  7,  3,  13, 8,  19, 4,  25, 14, 28, 9,  34, 20, 40,
...
MXC_FORCEINLINE
auto constexpr close(float const x, float const y, float const rel_tol = 1e-9f, float const abs_tol = 1e-8f) -> bool
{
    MMATH_ASSERT_FINITE_NORMALIZED_FLOAT(x);
    MMATH_ASSERT_FINITE_NORMALIZED_FLOAT(y);
